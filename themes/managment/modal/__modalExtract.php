...
<div class="p-8 flex flex-col gap-4">
      ...
      <header class="flex flex-col items-start pb-4">
            <h1 class="text-gray-800 text-xl font-semibold"><?= $recharge[0]->name_benefit; ?></h1>
            <p class="text-gray-500 text-md"><?= $recharge[0]->cpf; ?></p>
      </header>
            ...
            <div class="overflow-x-auto max-h-[310px]">
                  <table class="min-w-full divide-y divide-gray-200">
                        ...
                        <?php if (!empty($recharge)): ?>
                              <?php foreach($recharge as $rechargeIten): ?>
                                    <!-- Row -->
                                    <tr class="hover:bg-gray-50">
                                          <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900"><?=  fncstr_price($rechargeIten->value);  ?></td>
                                          <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500"><?=  fncMonthString($rechargeIten->month_recharge); ?></td>
                                          <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500"><?=  $rechargeIten->year_recharge;  ?></td>
                                          <td class="px-6 py-4 whitespace-nowrap">
                                                <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-green-100 text-green-800"><?=  $rechargeIten->status_recharge;  ?></span>
                                          </td>
